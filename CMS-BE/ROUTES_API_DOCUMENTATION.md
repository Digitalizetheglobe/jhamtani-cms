...
- [Gallery Photos API](#gallery-photos-api)
...
### Toggle Active Status
```http
PATCH /youtube-videos/{id}/toggle-status
```
  ...
  "data": [
    {
      ...
      "rating": 5,
      "testimonialText": "Amazing service and support team!",
      "photo": "photo-1755676660369-sarah-johnson.jpg",
...
```json
{
  ...
  "rating": 5,
  "testimonialText": "Outstanding quality and professional service!",
  "companyName": "Innovation Labs",
  "photo": "[file upload]"
}
```

**Note:** The `photo` field is optional. If provided, it should be an image file (JPG, PNG, etc.) with a maximum size of 5MB.
...
```json
{
  ...
  "rating": 4,
  "testimonialText": "Updated testimonial content",
  "companyName": "Updated Company",
  "photo": "[file upload]"
}
```

**Note:** The `photo` field is optional. If provided, it will replace the existing photo. The old photo file will be automatically deleted.
...
- `category` (string): Filter by category (festivals, exhibitions, outings, happy clients)
  ...
  "photos": [
    {
      ...
      "title": "",
      "description": "",
      "imageUrl": "/uploads/gallery/gallery-1234567890-image.jpg",
      "altText": "",
      "category": "festivals",
...
```json
{
  "categories": ["festivals", "exhibitions", "outings", "happy clients"]
}
```
...
```json
{
  ...
  "title": "",
  "description": "",
  "imageUrl": "/uploads/gallery/gallery-1234567890-festival.jpg",
  "altText": "",
  "category": "festivals",
  "tags": [],
  "isActive": true,
...
#### Get Festival Photos
```http
GET /gallery-photos/festivals
```

#### Upload Exhibition Photo
```http
POST /gallery-photos/exhibitions
```

#### Get Exhibition Photos
```http
GET /gallery-photos/exhibitions
```

#### Upload Outing Photo
```http
POST /gallery-photos/outings
```

#### Get Outing Photos
```http
GET /gallery-photos/outings
```

#### Upload Happy Client Photo
```http
POST /gallery-photos/happy-clients
```

#### Get Happy Client Photos
```http
GET /gallery-photos/happy-clients
```

### Generic Gallery Photo Operations
...
```
image: [file upload - required]
category: [required - one of: festivals, exhibitions, outings, happy clients]
title: [optional]
description: [optional]
altText: [optional]
tags: [optional - comma separated]
...
#### Toggle Active Status
```http
PATCH /gallery-photos/{id}/toggle-status
```
...
2. **Categories:**
   - Only four categories are supported: festivals, exhibitions, outings, happy clients
   - Category is required for all uploads

3. **Optional Fields:**
   - Title and description are optional for all categories
   - You can upload images without providing title or description
   - Empty strings will be used as defaults if not provided

4. **File Storage:**
   - Images are stored in `/uploads/gallery/` directory
   - File naming format: `gallery-{timestamp}-{originalname}`
...
```json
{
  "url": "https://api.risingspaces.in/blogimages/image-1234567890.jpg",
