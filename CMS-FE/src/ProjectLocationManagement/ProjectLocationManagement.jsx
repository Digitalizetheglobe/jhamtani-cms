...
const ProjectLocationManagement = () => {
  ...
  const fetchItems = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        response = await fetch(`${API_BASE_URL}/api/project-locations?limit=100`);
      }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const url = editing
        ? `${API_BASE_URL}/api/project-locations/${editing._id}`
        : `${API_BASE_URL}/api/project-locations`;
  ...
  const handleDelete = async (item) => {
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/api/project-locations/${item._id}`, {
        method: 'DELETE',
      });
  ...
  const handleToggle = async (item) => {
    try {
      const response = await fetch(
        `${API_BASE_URL}/api/project-locations/${item._id}/toggle-status`,
        { method: 'PATCH' }
      );
